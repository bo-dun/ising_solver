 ...
 */
int peek_poke_example(uint32_t a, uint32_t b, int slot_id, int pf_id, int bar_id);
 ...
 */
int peek_poke_example(uint32_t a, uint32_t b, int slot_id, int pf_id, int bar_id) {
    printf("entered peek_poke example");
    int rc;
...
printf("Writing 0x%08x to HELLO_WORLD register (0x%016lx)\n", value, HELLO_WORLD_REG_ADDR);*/
    rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, a);

    fail_on(rc, out, "Unable to write a to the fpga !");
    rc = fpga_pci_poke(pci_bar_handle, HELLO_WORLD_REG_ADDR, b);

    fail_on(rc, out, "Unable to write b to the fpga !");
    /* read it back and print it out; you should expect the byte order to be
     * reversed (That's what this CL does) */
    rc = fpga_pci_peek(pci_bar_handle, HELLO_WORLD_REG_ADDR, &a);
    fail_on(rc, out, "Unable to read read from the fpga !");
    printf("=====  Entering peek_poke_example =====\n");
    printf("MATRIX MULTIPLICATION RESULT: 0x%x\n", a);
/*    if(value == expected) {
        printf("TEST PASSED");
